...
var scanimg = {
  ...
  getImages: function (onlyTopFrame) {
    ...
    if (allFrames) allFrames = !onlyTopFrame;
    scanimg.allimages = new Object();
    //scanimg.monitorStatus(onlyTopFrame);
    //reset old results
    var ihtml =
      '<div id="spinner" class="spinner"><span id="processedImages" style="position: absolute;top: -38px;left: 50%;transform:translateX(-50%);white-space:nowrap;"></span><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>';
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      chrome.tabs.executeScript(
        tabs[0].id,
        { file: "app.js", allFrames: allFrames },
        scanimg.getImagesCallback
      );
    });
  },
  getImagesCallback: function (results) {
    if (scanimg.initiated) return;
    console.log(results);
    scanimg.initiated = true;
    if (chrome.extension.lastError || !results) {
      var message = chrome.extension.lastError
        ? chrome.extension.lastError.message
        : "Cannot access images in this page.";
      if (
        message.indexOf("The extensions gallery cannot be scripted") != -1 ||
        message.indexOf("Missing host permission for the tab") != -1
      ) {
      ...
      numImagesFoundDiv.origMessage =
        numImagesFoundDiv.origMessage || numImagesFoundDiv.innerHTML;
      numImagesFoundDiv.innerHTML = numImagesFoundDiv.origMessage.replace(
        "%n",
        document.querySelectorAll(".imgContainer").length
      );
      ...
      for (var r = 0; r < results.length; r++) {
        if (
          results[r] != null &&
          results[r].images != null &&
          results[r].images.length > 0
        ) {
      ...
      $("#processedImages")[0].innerHTML =
        "analyzing 0 of " + totalImages + " images";

      for (var r = 0; r < results.length; r++) {
        if (results[r] == null || results[r].images == null) continue;
        localStorage.active_tab_origin = results[r].origin;
        var images = results[r].images;
        console.log("images length: " + images.length);
        if (results[r].isTop) scanimg.title = results[r].title;
        for (var i = 0; i < images.length; i++) {
          try {
            ...
            img.onload = async function () {
              ...
              var style =
                "style='wwidth:" + width + "px;hheight:" + height + "px'";
              div.innerHTML =
                "<img class='origImg' id='" +
                imgSrc +
                "' src='" +
                imgSrc +
                "' " +
                style +
                "><div class='imgInfo'><div class='imgActions'><img class='zoomIcon' title='" +
                chrome.i18n.getMessage("Scan for deepfake") +
                "' src='" +
                chrome.extension.getURL("scan.png") +
                "' /></div><div class='imgSize imgDimension' style='margin-left:5px;'>-</div><div class='imgSize'>" +
                this.width +
                "x" +
                this.height +
                "</div><div class='imgSize imgType' style='margin-right:5px;'>other</div></div>";
              div.setAttribute("imgsrc", imgSrc);

              /*find the right place for appending based on pixels*/
              var currentImagesDivs =
              ...
              div.onclick = function (event) {
                if (event.target.className == "zoomIcon") {
                    ...
                    if(!imgSrc.startsWith("data", 0)){
                        console.log("url here");
                       getBase64FromUrl(imgSrc).then((data) => {
                            imgSrc = data;
                       });
                    }
                    console.log(imgSrc);
                ...
                //   sending base-64 data starts here
                
                  const send_data = { name: imgSrc, age: 234 };

                  fetch("http://127.0.0.1:8000/api/person/", {
                    method: "POST", // or 'PUT'
                    headers: {
                      "Content-Type": "application/json",
                      Authorization: "e31430543abe8cf509b9096ad5d9fae80ce6e85f",
                    },
              ...
              if (processedImages == totalImages) scanimg.foundlastimage();
            };
          } catch (e) {
            console.log(e);
          }
  ...
  selectImg: function (div, force, forceValue) {
    ...
    for (var k in scanimg.selection) {
      if (scanimg.selection.hasOwnProperty(k)) selectionLength++;
    }
  ...
  init: function () {
    ...
    $(document.body).on("click", function (event) {
      $(".filters div").each(function () {
        ...
        $(event.target)
          .parents()
          .each(function () {
            if (this == menuEl) clickedOnMenu = true;
          });
    ...
    $(".typeMenu").on("click", function (event) {
      ...
      if (conf.imgtype.indexOf("any") == 0) conf.imgtype = null;
  ...
  getConf: function (callback) {
    chrome.storage.sync.get(
      ...
      function (items) {
        ...
        var howoftentoshowrating =
          scanimg.numviews > 1000
            ? 100000
            : scanimg.numviews > 100
            ? 1000
            : scanimg.numviews > 10
            ? 100
            : 10;
  ...
  foundlastimage: function () {
    chrome.runtime.getBackgroundPage(function (bgWindow) {
      if (conf.savesize) scanimg.saveSizeConf();
        ...
        try {
          ...
          if (!mimeType) {
            if (imgSrc.indexOf("data:image/") == 0) {
              ...
              mimeType = imgSrcNoParams
                .substring(
                  imgSrcNoParams.lastIndexOf(".") + 1,
                  imgSrcNoParams.length
                )
                .toLowerCase();
            }
          }
      ...
      var typeSelector =
        ".imgContainer:not(.excludedUrl):not(.excludedSize):not(.excludedLayout)";
      ...
      $(".typeMenu")
        ...
        .each(function () {
          ...
          this.style.setProperty(
            "color",
            currTypeNum ? "inherit" : "lightGray",
            currTypeNum ? "" : "important"
          );
        });
      ...
      numImagesFoundDiv.innerHTML = numImagesFoundDiv.origMessage.replace(
        "%n",
        document.querySelectorAll(".imgContainer:not(.excluded)").length
      );
      document.getElementById("searchingimages").style.display = "none";
      document.getElementById("numimagesfound").style.display = "";
    });
  ...
  localize: function () {
    ...
    for (var i = 0; i < spans.length; i++) {
      var message = chrome.i18n.getMessage(
        spans[i].getAttribute("messagesKey")
      );
      if (message) spans[i].innerHTML = message;
    }
  },
};
